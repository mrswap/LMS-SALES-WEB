...
//   // Check if retry should be shown: NOT passed AND remaining attempts available
//   const canRetry = !isPassed && attemptsRemaining > 0;

//   // Get chapter ID from context for navigation
//   const currentChapterId = quizResults?.context?.chapter?.id;

//   const getScoreMessage = () => {
//     if (pct >= 80) return "Outstanding! Exceptional performance!";
//     if (pct >= 70) return "Excellent! Great work!";
//     if (pct >= 60) return "Good job! Keep it up!";
//     if (pct >= 50) return "Satisfactory! Room for improvement!";
//     if (pct >= 40) return "Fair effort! Keep practicing!";
//     return "Keep practicing! You'll do better next time!";
//   };

//   const getScoreSubtext = () => {
//     if (pct >= 80) return "You've demonstrated mastery of the subject matter.";
//     if (pct >= 70)
//       return "Strong understanding with minor improvements needed.";
//     if (pct >= 60) return "Solid foundation with some concepts to review.";
//     if (pct >= 50) return "Basic understanding achieved, keep practicing.";
//     if (pct >= 40)
//       return "Review the material and try again for better results.";
//     return "Don't give up! Review the levels and attempt the exam again.";
//   };
...
//   const handleRetry = () => navigate("/levels");
...
//           <p className="text-gray-600 text-lg">No exam result found.</p>
//           <button
...
//           {/* ── Analysis Card with Attempt Details ── */}
...
//             {/* Rows with attempt details */}
//             <div className="px-6">
//               {analysisRows.map(
//                 ({ icon: Icon, label, value, highlight, last }) => (
//                   <div
//                     key={label}
...
//                     </div>
//                     <span
//                       className={`qr-serif font-semibold text-[0.95rem] ${
...
//                     className="submit-btn w-full py-3.5 rounded-xl border-0 font-semibold text-[0.95rem] text-white flex items-center justify-center gap-2 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
//                     style={{
...
//                 Your input helps us improve our exams.
//               </p>
...
//           {/* ── Action Buttons (shown when feedback NOT submitted) ── */}
//           {!feedbackSubmitted && (
//             <div className="flex gap-4 justify-center flex-wrap animate-[fadeUp_.5s_ease_.45s_both]">
//               <button
//                 onClick={handleGoHome}
...
const ExamResult = () => {
  ...
  const getScoreMessage = () => {
    if (pct >= 80) return t("examResult.messages.outstanding");
    if (pct >= 70) return t("examResult.messages.excellent");
    if (pct >= 60) return t("examResult.messages.good");
    if (pct >= 50) return t("examResult.messages.satisfactory");
    if (pct >= 40) return t("examResult.messages.fair");
    return t("examResult.messages.keepPracticing");
  };

  const getScoreSubtext = () => {
    if (pct >= 80) return t("examResult.messages.outstandingSub");
    if (pct >= 70) return t("examResult.messages.excellentSub");
    if (pct >= 60) return t("examResult.messages.goodSub");
    if (pct >= 50) return t("examResult.messages.satisfactorySub");
    if (pct >= 40) return t("examResult.messages.fairSub");
    return t("examResult.messages.keepPracticingSub");
  };

  const getRatingLabel = (r) =>
    ({
      1: t("examResult.feedback.ratingLabels.1"),
      2: t("examResult.feedback.ratingLabels.2"),
      3: t("examResult.feedback.ratingLabels.3"),
      4: t("examResult.feedback.ratingLabels.4"),
      5: t("examResult.feedback.ratingLabels.5"),
    })[r] || "";
  ...
  const handleFeedbackSubmit = async (e) => {
    e.preventDefault();
    if (!feedback.rating) {
      setFeedbackError(t("examResult.errors.ratingRequired"));
      return;
    }
    setIsSubmittingFeedback(true);
    setFeedbackError("");
    try {
      ...
      setFeedbackError(
        err?.response?.data?.message || t("examResult.errors.feedbackFailed"),
      );
    } finally {
      setIsSubmittingFeedback(false);
    }
  ...
  if (!quizResults) {
    return (
      <PageLayout>
        <div className="flex flex-col justify-center items-center min-h-[60vh] gap-4">
          <p className="text-gray-600 text-lg">{t("examResult.noResult")}</p>
          <button
            onClick={handleGoHome}
            className="action-btn flex items-center gap-2 px-6 py-2.5 rounded-xl border-2 border-gray-200 bg-white text-gray-700 font-semibold text-sm cursor-pointer "
          >
  ...
  const stats = [
    ...
    },
    {
      icon: FiXCircle,
      value: quizResults.wrong || 0,
      label: t("examResult.stats.incorrect"),
      bg: "#ffe4e6",
      color: "#e11d48",
    },
    {
    ...
    },
    {
      icon: FiTrendingUp,
      value: `${Math.floor(quizResults.time_taken_minutes || 0)}m ${Math.floor((quizResults.time_taken_seconds || 0) % 60)}s`,
      label: t("examResult.stats.timeSpent"),
      bg: "#dbeafe",
      color: "#1d4ed8",
      small: true,
    },
  ...
  const analysisRows = [
    {
      icon: FiHash,
      label: t("examResult.analysis.attemptId"),
      value: `#${attemptIdValue}`,
    },
    {
      icon: FiBookOpen,
      label: t("examResult.analysis.totalQuestions"),
      value: quizResults.total || 0,
    },
    {
      icon: FiTarget,
      label: t("examResult.analysis.questionsAttempted"),
      value: quizResults.answered_questions || 0,
    },
    {
      icon: FiClock,
      label: t("examResult.analysis.pendingQuestions"),
      value: quizResults.remaining_questions || 0,
    },
    {
      icon: FiRefreshCw,
      label: t("examResult.analysis.attemptsUsed"),
      value: `${attemptsUsed} / ${totalAttemptsAllowed}`,
    },
    {
      icon: FiAward,
      label: t("examResult.analysis.attemptsRemaining"),
      value: attemptsRemaining,
      highlight: attemptsRemaining > 0 && !isPassed,
    },
    {
      icon: FiClock,
      label: t("examResult.analysis.totalDuration"),
      value: `${(quizResults.time_taken_minutes || 0).toFixed(2)} minutes`,
      last: true,
    },
  ];
        ...
        <PageHeaderLeft>
          <div className="flex items-center gap-5">
            ...
            <div className="animate-[fadeUp_.5s_ease_both]">
              <PageTitle className="qr-serif text-[1.75rem]">
                {isPassed
                  ? t("examResult.pageTitlePassed")
                  : t("examResult.pageTitleFailed")}
              </PageTitle>
              <PageSubtitle className="text-gray-500 mt-1">
                {isPassed
                  ? t("examResult.pageSubtitlePassed")
                  : t("examResult.pageSubtitleFailed")}
              </PageSubtitle>
            </div>
          </div>
        </PageHeaderLeft>
        ...
        <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm animate-[fadeUp_.5s_cubic-bezier(.22,.68,0,1.2)_.3s_both]">
              ...
              <div className="p-6">
                <form onSubmit={handleFeedbackSubmit}>
                  ...
                  <div className="mb-7">
                    <label className="block text-gray-700 font-medium text-sm mb-3">
                      {t("examResult.feedback.ratingLabel")}{" "}
                      <span className="text-rose-500">
                        {t("examResult.feedback.required")}
                      </span>
                    </label>
                  ...
                  <div className="mb-6">
                    <label className="block text-gray-700 font-medium text-sm mb-2">
                      {t("examResult.feedback.additionalComments")}
                    </label>
                    <textarea
                      value={feedback.review}
                      onChange={(e) =>
                      ...
                      rows={4}
                      placeholder={t("examResult.feedback.placeholder")}
                      className="qr-textarea w-full px-4 py-3 border-[1.5px] border-gray-200 rounded-xl text-sm text-gray-700 resize-none bg-gray-50 transition-all duration-200"
                      style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}
                    />
                  </div>
                  ...
                  >
                    {isSubmittingFeedback ? (
                      <>
                        ...
                        />
                        {t("examResult.feedback.submitting")}
                      </>
                    ) : (
                      <>
                        <FiSend className="w-4 h-4" />{" "}
                        {t("examResult.feedback.submitButton")}
                      </>
                    )}
                  </button>
                </form>
          ...
          {feedbackSubmitted && (
            ...
            >
              ...
              <h3 className="qr-serif text-[1.4rem] font-bold text-emerald-900 mb-1.5">
                {t("examResult.feedback.thankYouTitle")}
              </h3>
              <p className="text-emerald-700 text-sm mb-4">
                {t("examResult.feedback.thankYouMessage")}
              </p>
          ...
          {!feedbackSubmitted && (
            <div className="flex gap-4 justify-center flex-wrap animate-[fadeUp_.5s_ease_.45s_both]">
              ...
              {/* {isPassed && (
                ...
                >
                  <FiAward className="w-3.5 h-3.5" />
                  {t("examResult.buttons.getCertificate")}
                </button>
              )} */}
              ...
              {canRetry && (
                ...
                >
                  <FiRefreshCw className="w-3.5 h-3.5" />
                  {t("examResult.buttons.retry")} ({attemptsRemaining}{" "}
                  {t("examResult.buttons.attemptsLeft")})
                </button>
              )}
            </div>
          )}
