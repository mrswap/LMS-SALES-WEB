...
//   // Get main button config - MODIFIED to only handle continue/locked
//   const getMainButtonConfig = () => {
//     // For locked state - show locked
//     if (!item.is_unlocked) {
//       return {
//         text: t("levelsPage.buttons.locked"),
//         action: null,
...
//     // For unlocked levels - always show continue button
//     if (item.is_unlocked) {
//       return {
//         text: t("levelsPage.buttons.continue"),
//         action: () => navigate(`/levels/${item.id}`),
...
//         {/* Progress Bar - Yahi bar hai jo aayega */}
//         {item.is_unlocked && (
//           <div className="mt-3 mb-4">
//             <div className="flex justify-between text-xs text-gray-600 mb-1.5">
//               <span className="font-medium flex items-center gap-1">
...
//         {/* COMMENTED OUT: Content Completion Message */}
//         {/* {item.is_content_completed && !item.is_passed && (
//           <div className="mt-3 mb-3 p-2.5 bg-green-50 rounded-md border-l-4 border-green-500">
//             <div className="flex items-center gap-2">
//               <FaCheckCircle className="w-4 h-4 text-green-600" />
...
//         {/* COMMENTED OUT: Certificate Earned Message */}
//         {/* {item.is_passed && (
//           <div className="mt-3 mb-3 p-2.5 bg-emerald-50 rounded-md border-l-4 border-emerald-500">
//             <div className="flex items-center gap-2">
//               <FaAward className="w-4 h-4 text-emerald-600" />
...
//         {/* COMMENTED OUT: Exam Not Available Message */}
//         {/* {item.is_content_completed &&
//           !item.can_take_exam &&
//           !item.is_passed && (
//             <div className="mt-3 mb-3 p-2.5 bg-gray-50 rounded-md border-l-4 border-gray-400">
...
//         {/* Button Group - FAQ + Continue/View buttons */}
//         <div className="mt-2 space-y-2">
//           {/* FAQ Button - Yahi FAQ button hai jo aayega */}
//           <button
//             onClick={(e) => {
//               e.stopPropagation();
//               if (item.is_unlocked) {
...
//           {/* Main Action Button (Continue/Locked) */}
//           <button
//             onClick={(e) => {
//               e.stopPropagation();
//               if (mainButton.action) {
...
const LevelCard = ({ item }) => {
  ...
  // Determine status based on new flags - COMMENTED FOR FUTURE USE
  // const getStatus = () => {
  //   if (item.is_passed) return "passed";
  //   if (item.is_content_completed && item.can_take_exam && !item.is_passed)
  //     return "readyForExam";
  ...
  // Get status text for display - UPDATED: Only In Progress or Completed
  const getStatusText = () => {
    // For right top corner - only show In Progress or Completed
    if (item.is_passed) return t("levelsPage.status.completed");
    if (item.is_unlocked && !item.is_passed)
      return t("levelsPage.status.inProgress");
    return "";
  };

  // Get status icon - UPDATED for new badges
  const getStatusIcon = () => {
    if (item.is_passed) return <FaCheckCircle className="w-3 h-3" />;
    if (item.is_unlocked && !item.is_passed)
      return <FaHourglassHalf className="w-3 h-3" />;
    return null;
  };

  // Get status color - UPDATED for new badges
  const getStatusColor = () => {
    if (item.is_passed)
      return "bg-emerald-100 text-emerald-700 border border-emerald-200";
    if (item.is_unlocked && !item.is_passed)
  ...
  // Get main button config - ONLY ONE BUTTON: View if completed, Continue if not completed
  const getMainButtonConfig = () => {
    // For locked state - show locked
    if (!item.is_unlocked) {
      return {
    ...
    // For completed/passed levels - show VIEW button
    if (item.is_passed) {
      return {
        text: t("levelsPage.buttons.view"),
        action: () => navigate(`/levels/${item.id}`),
        disabled: false,
        color:
    ...
    // For unlocked but not completed levels - always show CONTINUE button
    if (item.is_unlocked && !item.is_passed) {
      return {
        text: t("levelsPage.buttons.continue"),
        action: () => navigate(`/levels/${item.id}`),
  ...
  return (
    <div className="group bg-white rounded-xl overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      <div className="relative">
        ...
        {/* Status Badge - UPDATED: Only shows In Progress or Completed */}
        {getStatusText() && (
          <div
            className={`absolute top-3 right-3 text-xs px-3 py-1.5 rounded-md shadow-sm font-medium flex items-center gap-1.5 ${getStatusColor()}`}
          >
      ...
      <div className="p-5">
        ...
        {/* Button Group - ONLY FAQ + MAIN BUTTON (View/Continue) */}
        <div className="mt-2 space-y-2">
          {/* FAQ Button - Yahi FAQ button hai jo aayega */}
          <button
            onClick={(e) => {
          ...
          {/* ONLY ONE MAIN BUTTON - View if completed, Continue if not completed */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (mainButton.action) {
