...
const LevelCard = ({ item }) => {
  ...
  const getStatus = () => {
    ...
    if (item.is_unlocked && !item.is_completed) return "RUNNING";
    return "LOCKED";
  };
  ...
  // Get button text
  const getButtonText = () => {
    if (status === "COMPLETED") return "Take an Exam";
    if (status === "RUNNING") return "Continue";
    return "Start Level";
  };

  // Get modules info
  const getModulesInfo = () => {
    if (!item.modules || item.modules.length === 0) return "0 Modules";
      ...
      <div className="p-4">
        ...
        <button
          onClick={(e) => {
            ...
            } else if (getButtonText() === "Take an Exam") {
...
export default function LevelsPage() {
  ...
  // Debug: Log the entire state to see what we're getting
  console.log("Full course state:", courseData);

  // Extract levels from the correct path
  let levelsArray = [];
  let programTitle = "Pace Maker";
  ...
  if (courseData.levels && Array.isArray(courseData.levels)) {
    ...
    // Case 2: levels contains a program object with levels property
      ...
      programTitle = courseData.levels[0].title || "Pace Maker";
    ...
    // Case 3: levels is the program object itself
      ...
      programTitle = courseData.levels.title || "Pace Maker";
  ...
  // Case 4: courseData might have a data property (API response wrapper)
    ...
    programTitle = courseData.data[0].title || "Pace Maker";
  ...
  console.log("Final levelsArray:", levelsArray);
      ...
      <PageBody>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            >
              Completed ({levelsArray.filter((l) => l.is_completed).length})
            </button>
          </div>
          ...
          {filteredLevels.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-gray-500 text-sm">
                {activeTab === "COMPLETED"
                  ? "No completed levels yet. Keep learning!"
                  : `No levels available. Levels found: ${levelsArray.length}`}
              </p>
            </div>
          )}
        </div>
