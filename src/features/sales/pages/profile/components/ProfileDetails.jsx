...
//                     {imageFile && currentImage === imagePreview && (
//                       <p className="text-sm text-blue-600">
//                         {t("profileDetails.profileImage.newImageSelected")}{" "}
//                         {imageFile.name}
//                       </p>
//                     )}
//                   </div>
...
const ProfileDetails = () => {
  ...
  const [imageFile, setImageFile] = useState(null);
  const [imagePreview, setImagePreview] = useState(null);
  const [isImageRemoved, setIsImageRemoved] = useState(false); // NEW: Track removal explicitly
  const toast = useToast();
  const { t } = useTranslation();
  ...
  const handleImageSelect = (e) => {
    ...
    if (file) {
      ...
      const previewUrl = URL.createObjectURL(file);
      setImagePreview(previewUrl);
      setIsImageRemoved(false); // Reset removal flag when new image selected
    }
  };
  ...
  const handleImageRemove = () => {
    setImageFile(null);
    setImagePreview(null);
    setIsImageRemoved(true); // IMPORTANT: Mark that image should be removed
  };
  ...
  const onSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (imageFile) {
        ...
        // FIXED: Image was explicitly removed
        formData.append("remove_image", "true");
        formData.append("profile_image", ""); // Send empty string for backend to handle
      }
  ...
  // Get current image to display - FIXED: Prioritize preview, then profile, but not if removed
  const currentImage = () => {
            ...
            <Form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="border-b border-gray-300 pb-6 mb-4">
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="relative group">
                    ...
                    {/* Hover overlay for image actions - FIXED: Only show if there's an image */}
                    {currentImage() && (
                      <div className="absolute inset-0 bg-black bg-opacity-50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <label className="cursor-pointer p-2 bg-white rounded-full hover:bg-gray-100 transition">
                          <input
                    ...
                    {/* If no image, show upload on hover */}
                    {!currentImage() && (
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={() => {
                    // FIXED: Properly reset all image states on cancel
                    setImageFile(null);
                    setImagePreview(null);
                    setIsImageRemoved(false);
                    resetForm();
