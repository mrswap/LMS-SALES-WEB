...
const ClassicAudioPlayer = ({
  ...
  // Cleanup audio when component unmounts or URL changes
  useEffect(() => {
    return () => {
      if (audioRef.current) {
        audioRef.current.pause();
...
const TopicContent = () => {
  ...
  useEffect(() => {
    if (!isLoading && content?.id) {
      ...
      // Audio player ko band karne ke liye showAudioPlayer false karo
      // Jab content change ho toh audio player hide ho jaye
  ...
  const renderAudioContent = () => {
    ...
    // Unique key for audio player to force remount when content changes
